...
public class SearchFragment
        extends SearchBaseListFragment<SearchResult, ListExtractor.InfoItemsPage>
    ...
    private SearchEngine.Filter filter = SearchEngine.Filter.STREAM;
   ...
   // private View searchClear;
    ...
    public static HiveRecyclerView suggestionsRecyclerView;
    private ProgressBar loading_progress_bar;
    private HiveRecyclerView items_list;
    private TextView recordtip;
    private RelativeLayout searchline_layout;
    ...
    protected void initViews(View rootView, Bundle savedInstanceState) {
        ...
        items_list=rootView.findViewById(R.id.items_list);
        searchEditText = rootView.findViewById(R.id.editText);
        searchline_layout=rootView.findViewById(R.id.searchline_layout);
        recordtip=rootView.findViewById(R.id.recordtip);
        ...
        suggestionsRecyclerView.setLayoutManager(ms);
        searchEditText.setOnKeyListener(onKeyListener);


        if(Utils.isPkgInstalled(activity,"com.baidu.input_baidutv")){
            ContentResolver resolver = activity.getContentResolver();
            String inputMethod = Settings.Secure.getString(resolver,
                    Settings.Secure.DEFAULT_INPUT_METHOD);
            Utils.print(tag,"default="+inputMethod);
            if(!inputMethod.equals("com.baidu.input_baidutv/.ImeService")){
                switchInputMethod();
            }
    ...
    View.OnKeyListener onKeyListener=new View.OnKeyListener() {
        ...
        public boolean onKey(View view, int keyCode, KeyEvent event) {
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
                if(keyCode==KeyEvent.KEYCODE_DPAD_DOWN){//itemRoot
                   if(suggestionsPanel.getVisibility()==View.VISIBLE&&suggestionsRecyclerView.getChildAt(0)!=null){
                       downPosition=0;
    ...
    private void initSearchListeners() {
        ...
        searchEditText.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (isSuggestionsEnabled && errorPanelRoot.getVisibility() != View.VISIBLE&&loading_progress_bar.getVisibility()!=View.VISIBLE) {
                    showSuggestionsPanel();
                }
                if(loading_progress_bar.getVisibility()!=View.VISIBLE){
                    showKeyboardSearch();
                }
        ...
        searchEditText.setOnFocusChangeListener(new View.OnFocusChangeListener() {
            ...
            public void onFocusChange(View v, boolean hasFocus) {
                ...
                if (isSuggestionsEnabled && hasFocus && errorPanelRoot.getVisibility() != View.VISIBLE&&loadingProgressBar.getVisibility()!=View.VISIBLE) {
                    showSuggestionsPanel();
                    showKeyboardSearch();
                }
    ...
    private void showSuggestionsPanel() {
        ...
        animateView(searchline_layout, AnimationUtils.Type.LIGHT_SLIDE_AND_ALPHA, true, 200);
    }
    ...
    private void hideSuggestionsPanel() {
        ...
        animateView(searchline_layout, AnimationUtils.Type.LIGHT_SLIDE_AND_ALPHA, false, 200);
    }
    ...
    private void showKeyboardSearch() {
        ...
        if (searchEditText.requestFocus()) {
            searchline_layout.setVisibility(View.VISIBLE);
            infoListAdapter.clearStreamItemList();
    ...
    public void handleSuggestions(@NonNull final List<SuggestionItem> suggestions) {
        ...
        ArrayList<SuggestionItem> suggestionsNew = new ArrayList<>();
        for (SuggestionItem item : suggestions) {
            if (item.fromHistory) {
                suggestionsNew.add(item);
            }
        }

        if(suggestionsNew.size()==0){
            suggestionsRecyclerView.post(() -> suggestionListAdapter.setItems(suggestionsNew));
            searchline_layout.setVisibility(View.INVISIBLE);
        }else{
            searchline_layout.setVisibility(View.VISIBLE);
            suggestionsRecyclerView.post(() -> suggestionListAdapter.setItems(suggestionsNew));
        }
    ...
    private void switchInputMethod() {
        if (Utils.isPkgInstalled(activity, Constants.INPUT_MANAGER)) {
