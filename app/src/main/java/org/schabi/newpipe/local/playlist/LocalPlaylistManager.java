...
public class LocalPlaylistManager {
     ...
     * @param streams
     */
    public Maybe<Boolean> checkPlaylistFavorite(final long playlistId, final List<StreamEntity> streams) {
        Log.v("test","streamsid=="+streams.get(0).getUid()+",=="+streams.get(0).getUrl());
        return playlistStreamTable.checkStreamsIsExist(playlistId,streams.get(0).getUrl())
                .firstElement()
                .map(new Function<List<PlaylistStreamEntry>, Boolean>() {
                    @Override
                    public Boolean apply(List<PlaylistStreamEntry> playlistStreamEntries) throws Exception {
                        Log.v("test","size="+playlistStreamEntries.size());
                        return playlistStreamEntries.size()==0?false:true;
                    }
                })
                .subscribeOn(Schedulers.io());
    }
    ...
    public Maybe<List<Long>> appendToPlaylist(final long playlistId,
        ...
        return playlistStreamTable.getMaximumIndexOf(playlistId)
                .firstElement()
                .map(new Function<Integer, List<Long>>() {
                         @Override
                         public List<Long> apply(Integer maxJoinIndex) throws Exception {
                             Log.v("test","index=="+maxJoinIndex);
                             return database.runInTransaction(() ->
                                     LocalPlaylistManager.this.upsertStreams(playlistId, streams, maxJoinIndex + 1));
                         }
                     }
                ).subscribeOn(Schedulers.io());
    }
