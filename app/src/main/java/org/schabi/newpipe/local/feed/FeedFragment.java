...
public class FeedFragment extends FeedBaseListFragment<List<SubscriptionEntity>, Void> {
    ...
    public void onResume() {
        super.onResume();
        subscriblayout.setVisibility(View.GONE);
    ...
    public void startLoading(boolean forceLoad) {
        ...
        if (allItemsLoaded.get()) {
            if (feedinfoListAdapter.getItemsList().size() == 0) {
                showEmptyState();
            } else {
        ...
        subscriptionService.getSubscription().toObservable()
                ...
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(getSubscriptionObserverSub());
    }
    private Observer<List<SubscriptionEntity>> getSubscriptionObserverSub() {
    ...
    public void handleResultSub(@NonNull List<SubscriptionEntity> result) {
    ...
    public void handleResult(@android.support.annotation.NonNull List<SubscriptionEntity> result) {
        super.handleResult(result);
        if (result.isEmpty()) {
            feedinfoListAdapter.clearStreamItemList();
            showEmptyState();
            return;
        }
    ...
    private Subscriber<SubscriptionEntity> getSubscriptionObserver() {
        return new Subscriber<SubscriptionEntity>() {
            ...
            public void onSubscribe(Subscription s) {
                ...
                int requestSize = FEED_LOAD_COUNT - feedinfoListAdapter.getItemsList().size();
                if (wasLoading.getAndSet(false)) requestSize = FEED_LOAD_COUNT;

                boolean hasToLoad = requestSize > 0;
                if (hasToLoad) {
                    requestLoadedAtomic.set(feedinfoListAdapter.getItemsList().size());
    ...
    private MaybeObserver<ChannelInfo> getChannelInfoObserver(final int serviceId, final String url) {
        return new MaybeObserver<ChannelInfo>() {
            ...
            public void onSuccess(final ChannelInfo channelInfo) {
                if (feedinfoListAdapter == null || channelInfo.getRelatedItems().isEmpty()) {
                    onDone();
                    return;
                }
                ...
                boolean itemExists = doesItemExist(feedinfoListAdapter.getItemsList(), item);
                if (!itemExists) {
                    subscriblayout.setVisibility(View.VISIBLE);
                    ...
                    feedinfoListAdapter.addInfoItem(item);
    ...
    private void resetFragment() {
        ...
        if (feedinfoListAdapter != null) feedinfoListAdapter.clearStreamItemList();
        if (disposables != null) disposables.clear();
