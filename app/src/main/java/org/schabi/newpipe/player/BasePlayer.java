...
import static org.schabi.newpipe.player.MainVideoPlayer.hasChanged;
...
public abstract class BasePlayer implements
    ...
    private Disposable getAllHistory;
    private Disposable onDelete;
    ...
    public void destroyPlayer() {
        ...
        Log.d(TAG, "simpleExoPlayer ==" + (simpleExoPlayer == null));
        if (simpleExoPlayer != null) {
            simpleExoPlayer.removeListener(this);
            simpleExoPlayer.stop();
            simpleExoPlayer.release();
            Log.d(TAG, "simpleExoPlayer release");
    ...
    public void onTimelineChanged(Timeline timeline, Object manifest,
        ...
        switch (reason) {
            ...
            case Player.TIMELINE_CHANGE_REASON_DYNAMIC: // called after playlist changes
                ...
                Log.i("QQQQ", "isPlaylistStable:" + isPlaylistStable + ":isCurrentWindowValid():" + isCurrentWindowValid() + ":isSynchronizing:" + isSynchronizing);
    ...
    public void onPlayPrevious() {
        ...
        if (playQueue.getStreams().size() > 1) {
            Toast.makeText(context, context.getResources().getString(R.string.main_player_previous), Toast.LENGTH_SHORT).show();
        }
        savePlaybackState();
        playQueue.offsetIndex(-1);
    }
    ...
    public void onPlayNext() {
        ...
        if (playQueue.getStreams().size() > 1) {
            Toast.makeText(context, context.getResources().getString(R.string.main_player_next), Toast.LENGTH_SHORT).show();
        }
        savePlaybackState();
        playQueue.offsetIndex(+1);
    }
    ...
    private void registerView() {
        ...
        limitMaxHistoryStream();
        //Log.v("test","registerView");
    }
    ...
    private void limitMaxHistoryStream() {
        getAllHistory = recordManager.getStreamStatistics()
                .observeOn(Schedulers.io())
                .subscribe(new Consumer<List<StreamStatisticsEntry>>() {
                    @Override
                    public void accept(List<StreamStatisticsEntry> streamStatisticsEntries) throws Exception {
                        //临时规避，未能确定
                        if (getAllHistory != null)
                            getAllHistory.dispose();
                        List<StreamStatisticsEntry> result = processResult(streamStatisticsEntries);
                        Log.v("test", "history size=" + result.size());
                        if (!recordManager.checkStreamExistHistory(currentInfo) && result.size() >= HistoryRecordManager.MAX_LIMIT) {
                            //delete last one
                            deleteEntry(result.get(result.size() - 1));
                        } else {
                            addToPlayHistory();
                        }
                    }
    ...
    /*private Subscriber<List<StreamStatisticsEntry>> getHistoryObserver() {
        return new Subscriber<List<StreamStatisticsEntry>>() {
            ...
            public void onNext(List<StreamStatisticsEntry> streams) {
                List<StreamStatisticsEntry> result = processResult(streams);
                Log.v("test","history size="+result.size());
                if(result.size()>=HistoryRecordManager.MAX_LIMIT){
                    //delete last one
                    deleteEntry(result.get(result.size()-1));
                }
    ...
    private void deleteEntry(StreamStatisticsEntry entry) {
        Log.v("test", "deleteEntry");
        onDelete = recordManager.deleteStreamHistory(entry.streamId)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Consumer<Integer>() {
                    @Override
                    public void accept(Integer howManyDelted) throws Exception {
                        onDelete.dispose();
                        Log.v("test", "delete sucess");
                        addToPlayHistory();
                    }
                });
    }
