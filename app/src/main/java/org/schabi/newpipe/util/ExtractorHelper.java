...
public final class ExtractorHelper {
    ...
    public static Single<ChannelInfo> getChannelInfo(final int serviceId,
        ...
        checkServiceId(serviceId);
        //规避尝试！！！！
        Single<ChannelInfo> infoSingle = new Single<ChannelInfo>() {
            @Override
            protected void subscribeActual(SingleObserver<? super ChannelInfo> observer) {

            }
        };
        try {
            infoSingle = checkCache(forceLoad, serviceId, url, Single.fromCallable(() ->
                    ChannelInfo.getInfo(NewPipe.getService(serviceId), url)));
        } catch (Exception e) {
            Toast.makeText(App.getInstance(), "服务器异常，无法找到当前订阅人的相关视频", Toast.LENGTH_SHORT).show();
        }
        return infoSingle;
    }
