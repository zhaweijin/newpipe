...
import org.schabi.newpipe.fragments.list.KioskBaseListFragment;
...
public class KioskInfoListAdapter extends HiveRecyclerView.Adapter<HiveRecyclerView.ViewHolder> {
    private static final String TAG = KioskInfoListAdapter.class.getSimpleName();
    ...
    private ReceiverSlidegrid receiverSlidegrid;
    ...
    public KioskInfoListAdapter(Activity a) {
        infoItemBuilder = new InfoItemBuilder(a);
        infoItemList = new ArrayList<>();
        this.activity=a;
        //注册广播
        receiverSlidegrid = new ReceiverSlidegrid();
        //实例化过滤器并设置要过滤的广播
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction("org.schabi.newpipe.slidegrid");
        a.registerReceiver(receiverSlidegrid, intentFilter);
    }
    ...
    public void onBindViewHolder(HiveRecyclerView.ViewHolder holder, int position) {
        ...
        //点击焦点回落的处理
        int clickPos=KioskBaseListFragment.clickPosition;
        if(getItemCount()>0&&clickPos<getItemCount()&&clickPos>-1&&position==clickPos){
            holder.itemView.setFocusable(true);
            holder.itemView.requestFocus();
            clickPos=KioskBaseListFragment.clickPosition=-1;
    ...
    View.OnKeyListener onKeyListener=new View.OnKeyListener() {
        ...
        public boolean onKey(View view, int keyCode, KeyEvent event) {
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
               /* if(keyCode==KeyEvent.KEYCODE_DPAD_RIGHT){//itemRoot
                    ...
                    int pos= KioskBaseListFragment.itemsList.getChildPosition(view);
                    if(pos<4){
                        MainFragment.isFocus=true;
                        MainFragment.layout_recommend.setFocusable(true);
                        MainFragment.layout_recommend.requestFocus();
                       Intent intent = new Intent("org.schabi.newpipe.slidegrid");
                    ...
                    MainFragment.layout_recommend.setFocusable(true);
                    MainFragment.layout_recommend.requestFocus();
                    KioskBaseListFragment.itemsList.smoothScrollToPosition(0);
                    return true;
                }
            }
            return false;
    ...
    public class StreamInfoItemHolder extends KioskStreamMiniInfoItemHolder {
    ...
    public class KioskStreamMiniInfoItemHolder extends InfoItemHolder {
        ...
        KioskStreamMiniInfoItemHolder(InfoItemBuilder infoItemBuilder, int layoutId, ViewGroup parent) {
        ...
        public KioskStreamMiniInfoItemHolder(InfoItemBuilder infoItemBuilder, ViewGroup parent) {
    ...
    public class ReceiverSlidegrid extends BroadcastReceiver {
        @Override
    ...
    public void unRegister(){
        try{
            if(receiverSlidegrid!=null){
                activity.unregisterReceiver(receiverSlidegrid);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
