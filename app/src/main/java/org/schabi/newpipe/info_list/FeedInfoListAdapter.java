...
public class FeedInfoListAdapter extends HiveRecyclerView.Adapter<HiveRecyclerView.ViewHolder> {
    private static final String TAG = FeedInfoListAdapter.class.getSimpleName();
    ...
    private ReceiverSlideNew receiverSlideNew;
    ...
    public FeedInfoListAdapter(Activity a) {
        infoItemBuilder = new InfoItemBuilder(a);
        infoItemList = new ArrayList<>();
        this.activity=a;
        //注册广播
        receiverSlideNew = new ReceiverSlideNew();
        //实例化过滤器并设置要过滤的广播
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction("org.schabi.newpipe.slidenew");
        a.registerReceiver(receiverSlideNew, intentFilter);
    }
    ...
    public HiveRecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int type) {
        ...
        switch (type) {
            ...
            case STREAM_HOLDER_TYPE:
                return new FeedStreamInfoItemHolder(infoItemBuilder, parent);
    ...
    public void onBindViewHolder(HiveRecyclerView.ViewHolder holder, int position) {
        ...
        //返回时焦点位置需滑动显示的处理
      /*  if(position==0&&FeedBaseListFragment.clickPosition>3){
            FeedBaseListFragment.msfeed.scrollToPositionWithOffset(FeedBaseListFragment.clickPosition, 0);
            return;
        }*/
        //点击焦点回落的处理
        int clickPos=FeedBaseListFragment.clickPosition;
        if(getItemCount()>0&&clickPos<getItemCount()&&clickPos>-1&&position==clickPos){
            holder.itemView.setFocusable(true);
            holder.itemView.requestFocus();
            clickPos=FeedBaseListFragment.clickPosition=-1;
        ...
        public boolean onKey(View view, int keyCode, KeyEvent event) {
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
                if(keyCode==KeyEvent.KEYCODE_DPAD_LEFT){//itemRoot
                    int pos= FeedBaseListFragment.newList.getChildPosition(view);
                    if(pos==0){
                        return true;
                    }
                    ...
                    MainFragment.layout_subscribe.setFocusable(true);
                    MainFragment.layout_subscribe.requestFocus();
                    FeedBaseListFragment.msfeed.scrollToPositionWithOffset(0, 0);
                    //FeedBaseListFragment.msfeed.setStackFromEnd(true);
                    return true;
                }
            }
            return false;
    ...
    public class FeedStreamInfoItemHolder extends FeedStreamMiniInfoItemHolder {

        public final TextView itemAdditionalDetails;

        public FeedStreamInfoItemHolder(InfoItemBuilder infoItemBuilder, ViewGroup parent) {
            super(infoItemBuilder, R.layout.list_stream_item_feed, parent);
    ...
    public class FeedStreamMiniInfoItemHolder extends InfoItemHolder {
        ...
        FeedStreamMiniInfoItemHolder(InfoItemBuilder infoItemBuilder, int layoutId, ViewGroup parent) {
        ...
        public FeedStreamMiniInfoItemHolder(InfoItemBuilder infoItemBuilder, ViewGroup parent) {
    ...
    public class ReceiverSlideNew extends BroadcastReceiver {
        @Override
    ...
    public void unRegister(){
        try{
            if(receiverSlideNew!=null){
                activity.unregisterReceiver(receiverSlideNew);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
