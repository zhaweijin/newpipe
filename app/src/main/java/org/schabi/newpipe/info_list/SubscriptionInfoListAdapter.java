...
import org.schabi.newpipe.fragments.list.FeedBaseListFragment;
...
public class SubscriptionInfoListAdapter extends HiveRecyclerView.Adapter<HiveRecyclerView.ViewHolder> {
    private static final String TAG = SubscriptionInfoListAdapter.class.getSimpleName();
    ...
    private ReceiverSlideSubscription receiverSlideSubscription;
    ...
    public SubscriptionInfoListAdapter(Activity a) {
        infoItemBuilder = new InfoItemBuilder(a);
        infoItemList = new ArrayList<>();
        this.activity=a;
        //注册广播
        receiverSlideSubscription = new ReceiverSlideSubscription();
        //实例化过滤器并设置要过滤的广播
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction("org.schabi.newpipe.slidesubscription");
        a.registerReceiver(receiverSlideSubscription, intentFilter);
    }
    ...
    public HiveRecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int type) {
        ...
        switch (type) {
            ...
            case CHANNEL_HOLDER_TYPE:
                return new SubscriptionChannelInfoItemHolder(infoItemBuilder, parent);
    ...
    public void onBindViewHolder(HiveRecyclerView.ViewHolder holder, int position) {
        ...
        //返回时焦点位置需滑动显示的处理
        if(position==0&&FeedBaseListFragment.clickPosition_down>5){
            if(FeedBaseListFragment.clickPosition_down<getItemCount()){
                FeedBaseListFragment.ms.scrollToPositionWithOffset(FeedBaseListFragment.clickPosition_down, 0);
            }else{
                FeedBaseListFragment.ms.scrollToPositionWithOffset(getItemCount()-1, 0);
            }
            return;
        }
        //点击焦点回落的处理(因数据实时刷新，所以将焦点定位原来点击进入的位置)
        int clickPos=FeedBaseListFragment.clickPosition_down;
        if(getItemCount()>0&&clickPos<getItemCount()&&clickPos>-1&&position==clickPos){
            holder.itemView.setFocusable(true);
            holder.itemView.requestFocus();
            clickPos=FeedBaseListFragment.clickPosition_down=-1;
        ...
        public boolean onKey(View view, int keyCode, KeyEvent event) {
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
                if(keyCode==KeyEvent.KEYCODE_DPAD_LEFT){//itemRoot
                   int pos= FeedBaseListFragment.itemsList.getChildPosition(view);
                   if(pos==0){
                       return true;
                   }
                }else if(keyCode==KeyEvent.KEYCODE_DPAD_RIGHT){
                    int pos= FeedBaseListFragment.itemsList.getChildPosition(view);
                    if(pos==infoItemList.size()-1){
                        return true;
                    }

                }else if(keyCode==KeyEvent.KEYCODE_BACK){
                    MainFragment.layout_subscribe.setFocusable(true);
                    MainFragment.layout_subscribe.requestFocus();
                    FeedBaseListFragment.ms.scrollToPositionWithOffset(0, 0);
                   // FeedBaseListFragment.ms.setStackFromEnd(true);
                    return true;
                }
            }
            return false;
    ...
    public class SubscriptionChannelInfoItemHolder extends SubscriptionChannelMiniInfoItemHolder {
        // public final TextView itemChannelDescriptionView;

        public SubscriptionChannelInfoItemHolder(InfoItemBuilder infoItemBuilder, ViewGroup parent) {
            super(infoItemBuilder, R.layout.list_channel_item_new, parent);
    ...
    public class SubscriptionChannelMiniInfoItemHolder extends InfoItemHolder {
        public final CircleImageView itemThumbnailView;
        public final TextView itemTitleView;
        public final TextView itemAdditionalDetailView;

        SubscriptionChannelMiniInfoItemHolder(InfoItemBuilder infoItemBuilder, int layoutId, ViewGroup parent) {
        ...
        public SubscriptionChannelMiniInfoItemHolder(InfoItemBuilder infoItemBuilder, ViewGroup parent) {
      ...
      if(isLoadIconURL){
          ...
          itemThumbnailView.setImageResource(R.drawable.buddy_channel_item);
      }
    ...
    public class ReceiverSlideSubscription extends BroadcastReceiver {
        @Override
    ...
    public void unRegister(){
        try{
            if(receiverSlideSubscription!=null){
                activity.unregisterReceiver(receiverSlideSubscription);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
